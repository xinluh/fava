    ...
    def p_simple_expr_key(self, p: list[Any]) -> None:
        ...
        if key == 'amount':
            ...
            try:
                ...
                raise FilterException('amount', f'Failed to parse amount: {value}')
        ...
        def _key(entry: Directive) -> bool:
            ...
            if key == 'amount':
                return any(
                    posting.units.currency == value.currency and {
                        '=': posting.units == value,
                        '>=': posting.units >= value,
                        '<=': posting.units <= value,
                        '>': posting.units > value,
                        '<': posting.units < value,
                    }[amount_match_mode] for posting in getattr(entry, "postings", [])
                )
